        ...
        <form method="POST" action="{{ route('admin.cafes.store') }}" enctype="multipart/form-data">
            ...
            <div class="mb-4">
                ...
                <textarea name="description" rows="3"
                    ...
                    class="w-full border rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-amber-400 resize-none">{{ old('description') }}</textarea>
            </div>
            ...
            <div class="grid grid-cols-1 md:grid-cols-3 gap-4 mb-4">
                <div>
                    ...
                    <select name="wifi_quality"
                        ...
                        <option value="good"   @selected(old('wifi_quality')=='good')>📶 Good</option>
                        <option value="medium" @selected(old('wifi_quality')=='medium')>📶 Medium</option>
                        <option value="bad"    @selected(old('wifi_quality')=='bad')>📶 Bad</option>
                    </select>
                </div>
                <div>
                    ...
                    <select name="power_outlet"
                        class="w-full border rounded-lg px-3 py-2 text-sm focus:ring-2 focus:ring-amber-400">
                        <option value="">Pilih...</option>
                        <option value="many" @selected(old('power_outlet')=='many')>🔌 Banyak</option>
                        <option value="few"  @selected(old('power_outlet')=='few')>🔌 Sedikit</option>
                        <option value="none" @selected(old('power_outlet')=='none')>❌ Tidak Ada</option>
                    </select>
                </div>
                <div>
                    ...
                    <select name="noise_level"
                        class="w-full border rounded-lg px-3 py-2 text-sm focus:ring-2 focus:ring-amber-400">
                        <option value="">Pilih...</option>
                        <option value="quiet"    @selected(old('noise_level')=='quiet')>🤫 Quiet</option>
                        <option value="moderate" @selected(old('noise_level')=='moderate')>🔉 Moderate</option>
                        <option value="noisy"    @selected(old('noise_level')=='noisy')>🔊 Noisy</option>
                    </select>
                </div>
            </div>
